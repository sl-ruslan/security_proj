...
module.exports = async (state, params) => {
  if (state.user !== 'admin') { 
    logAction(state.user, `crights ${params}`, 'Permission denied');
    throw new Error('Permission denied');
  }
  if (params.length !== 1) { 
    logAction(state.user, `crights ${params}`, 'Incorrect params length');
  ...
  if (!existUserByLogin(allUsers, login)) { 
    logAction(state.user, `crights ${params}`, 'Unknown user');
  ...
  logAction(state.user, `crights ${params}`);
  save(state.disk);
};
